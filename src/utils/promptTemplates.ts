...
// --- REFERENCE SECTION (Kept shorter to reduce noise) ---
const REFERENCE_HTML_SNIPPET = `<!DOCTYPE html>
<html lang="en">
<head>
<script src="https://cdnjs.cloudflare.com/ajax/libs/gsap/3.12.2/gsap.min.js"></script>
<style>
    :root { --bg-deep: #050505; --primary: #00f3ff; }
    /* ... CSS Styles ... */
</style>
</head>
<body>
...
<script>
    const tl = gsap.timeline({ paused: true });
    // ... GSAP Animations ...
    window.addEventListener('message', (e) => { /* sync logic */ });
</script>
</body>
</html>`;
...
export const constructPrompt = (topic: string, srt: string) => {
    return `
You are an expert Creative Developer creating a specialized HTML overlay for a video.

---
### ⛔️ NEGATIVE CONSTRAINTS (CRITICAL)
1. **DO NOT** output the "Bloom Filter" example. That is just a code structure reference.
2. **DO NOT** use placeholders like "Lorem Ipsum". Use the actual keywords from the USER TOPIC below.
3. **DO NOT** make the animation length 10 seconds. It MUST match the approximate length of the provided transcript.

---
### 1. THE USER'S REQUEST (FOCUS HERE)

**Target Topic:** "${topic || "General Explanation"}"
*(Generate specific visuals, text, and metaphors related to THIS topic)*

**Transcript Context:**
${srt ? srt.slice(0, 1000) : "(No transcript provided, infer from topic)"} ... (truncated)

---
### 2. CODE REQUIREMENTS
Generate a **NEW** HTML file using **GSAP** based on the user's topic.

**Design System:**
- Background: Dark (#050505) or transparent (if overlaying video).
- Fonts: 'Oswald' (Headers), 'JetBrains Mono' (Data).
- Style: Cyberpunk / High-Tech / Clean.
...
1. \`html\`: The full HTML string.
2. \`layoutConfig\`: Array of layout steps (e.g. splitRatio: 0.6).
`;
};