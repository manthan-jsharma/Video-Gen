...
import { Edit3, AlertCircle, LayoutTemplate, CheckCircle2, Globe, Github, Linkedin, Instagram, Facebook, BookOpen, X, Sparkles } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const [apiKey, setApiKey] = useState(() => {
    ...
    return stored || APP_CONFIG.DEFAULT_API_KEY;
  });
  const [modelName, setModelName] = useState(APP_CONFIG.DEFAULT_MODEL);
  ...
  const handleGenerate = async () => {
     ...
     if (!apiKey.trim()) {
       setError("Please provide a Gemini API Key in the settings.");
       return;
     }
     ...
     setError(null);
     try {
       const content = await generateReelContent(srtTextRaw, topicContext, apiKey, modelName);
      ...
      <main className="flex-1 overflow-hidden relative flex flex-col">
        ...
        {appState === AppState.UPLOAD && (
          <div className="flex flex-col h-full overflow-y-auto">
             ...
             {/* Author / About Section - Moved to First Page */}
        ...
        {appState === AppState.GENERATING && (
          <div className="flex flex-col h-full overflow-auto">
              <div className="flex-1 flex flex-col items-center justify-center max-w-2xl mx-auto p-6 space-y-8 animate-fade-in">
                ...
                <div className="w-full space-y-3">
                    ...
                    {isGenerating ? (
                        ...
                        <>
                        <Edit3 size={18} />
                        {srtData.length === 0 ? "Enter Demo Studio" : "Enter Studio & Auto-Generate"}
                        </>
                    )}
                    </button>
            ...
            {/* Author Section Removed from here */}
          </div>
        )}
